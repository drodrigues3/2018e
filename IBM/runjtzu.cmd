@ECHO OFF
@echo *********** Welcome to the IBM Time Zone Update Utility for Java (JTZU) Version 1.7.18e ***********

rem set JTZU_HOME to the current directory
set JTZU_HOME=/* ENTER YOUR JTZU HOME PATH LIKE C:\DESKTOP\JTZU\ */
...
IF NOT EXIST "%JTZU_HOME%JTZU.jar" GOTO MissingJTZUENV
	call "%JTZU_HOME%runjtzuenv.cmd"
	@echo Java Home: %JAVA_HOME%
	GOTO Temp

:MissingJTZUENV
@echo runjtzuenv.cmd file doesn't exist.
GOTO Exit
...
@echo "%JAVA_HOME%\bin\java.exe" -cp "%JTZU_HOME%JTZU.jar" -Dnogui=%NOGUI% -Ddiscoveronly=%DISCOVERONLY% -Dsilentpatch=%SILENTPATCH% JTZUMain
"%JAVA_HOME%\bin\java.exe" -cp "%JTZU_HOME%JTZU.jar" -Dnogui=%NOGUI% -Ddiscoveronly=%DISCOVERONLY% -Dsilentpatch=%SILENTPATCH% JTZUMain
